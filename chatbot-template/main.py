...
def format_sources(source):
    ...
    if '_excerpt_page_number' in source.metadata['document_attributes']:
        page_number = " - Page " + str(source.metadata['document_attributes']['_excerpt_page_number'])
                                                    
    source_string = "**" + str(source.metadata['title']) + "**" + page_number + "\n" + str(source.metadata['source']) + "\n" 
...
if prompt := st.chat_input("Ask a question"):
    ...
    with st.chat_message("assistant"):
        ...
        with st.spinner('I\'m working on it...'):
            ...
            full_response, sources, intermediate_steps = run_chain(llm, chain, memory, prompt)
    ...
    st.session_state.messages.append({"role": "assistant", "content": full_response, "sources": sources, "intermediate_steps": intermediate_steps})
